...
def create_model(activation="relu", nb_hidden=75, layer_num=5, dropout=0.0, bn='false'):
  model = Sequential([
...
middle_layer_node_num = [175, 200, 225, 250]
middle_layer_num = [3, 4]
...
results.to_csv("allv2_grid4.csv")

scores = results.pivot("param_layer_num", "param_nb_hidden", "mean_test_score")
print(scores)

plt.figure()
sns.heatmap(scores, annot=True, fmt="1.3f")
plt.savefig('image/allv2_heatmap4_1600.png')
